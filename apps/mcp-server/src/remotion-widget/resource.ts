...
async function buildWidgetHtml(): Promise<string> {
  ...
  widgetHtmlPromise = (async () => {
    ...
    try {
      ...
      console.error("[remotion-widget] Widget build error:", error);
      throw new Error(
        `Failed to build Remotion widget: ${(error as Error).message}`
      );
    }
  })();
  ...
  server.registerResource(
    ...
    async () => {
      try {
        ...
        console.error("[remotion-widget] Failed to build widget HTML:", error);
        throw error;
      }
    },
  );
